...
{
    public class CustomCinemachineCamera : CinemachineVirtualCameraBase
    {
        ...
        public Unity.Cinemachine.CameraTarget Target;
        ...
        public override void InternalUpdateCameraState(Vector3 worldUp, float deltaTime)
        {
            ...
            var toNextPosition = m_State.RawPosition - transform.position;
            var ray = new Ray(transform.position, toNextPosition.normalized);
            var isHit = Physics.Raycast(ray, toNextPosition.magnitude * 2, collisionMask);
            if (isHit)
            {
                PreviousStateIsValid = false;
                return;
