...
/// * grid가 X/Y(가로/세로)로 일정간격으로 깔리되, Rect 경계 넘어서지 않음
/// </summary>
public class HatchGridGenerator : MonoBehaviour
{
    ...
    [FoldoutGroup("Settings"), Tooltip("Grid 간격")]
    ...
    [FoldoutGroup("Result"), ReadOnly]
    public int numX, numZ;  // 실제 계산된 해상도(칸수+1)
    ...
    public void GenerateHatchGrid()
    {
        ...
        {
            Debug.LogWarning("[HatchGridGenerator] boundingRect가 유효X");
            return;
        }
        ...
        {
            Debug.LogWarning("[HatchGridGenerator] 코스 데이터가 부족");
            return;
        }
        ...
        if(width <= 0 || height <= 0)
        {
            Debug.LogWarning("[HatchGridGenerator] Rect size 0 이하");
            return;
        }

        // numX, numZ : 실제 '칸' 개수 +1 = 정점 개수
        ...
        for(int iz = 0; iz < numZ; iz++)
        {
            ...
            for(int ix = 0; ix < numX; ix++)
            {
                ...
                // 만약 xVal, zVal 모두 xMax, zMax를 넘어서 
                // (즉, 마지막 칸에서 cellSize가 남지 않는 경우) 
                // => clamp 되었으므로 Rect 밖을 벗어나지 않음.
    ...
    /// <summary>
    /// 코스 내부 여부 (Point-In-Triangle), 2D판정
    /// </summary>
    ...
    /// <summary>
    /// 코스 주변 height : 
    /// 여기서는 "가장 가까운 DownhillNode"의 y값을 할당하는 간단 방식.
    /// </summary>
    private float ComputeHeightByCourse(Vector3 pos)
    {
        // DownhillNodes => pos.xz와 노드 position.xz 거리 최소
        // node의 position.y => height
