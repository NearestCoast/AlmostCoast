...
{
    public partial class HangJumpState : MovementState
    {
        public override StateType Type => StateType.HangJump;

        [SerializeField, TitleGroup("Animation")]
        private LinearMixerTransition anims;

        [SerializeField, TitleGroup("CollisionEffects")]
        private VisualEffect collisionVFX;

        [SerializeField, TitleGroup("CollisionEffects")]
        private AudioSource collisionSFX;

        [SerializeField, TitleGroup("CollisionEffects")]
        private HitObject collisionHitObject;
        ...
        public override void OnEnterState()
        {
            ...
            {
                InitialHeightSnap = (MoveParams.IsWallJumpable)
                    ? VerticalParams.WallPoint.Value.y
                    : GroundParams.GroundPoint.y;
            }
    ...
    public partial class HangJumpState : MovementState
    {
        ...
        protected override Vector3 GetVelocity()
        {
            ...
            if (connectedInput && !connectedInput.IsPressing &&
                transform.position.y - InitialHeightSnap > MaxJumpHeight * 0.5f)
            {
                IsLeapEnd = true;
            }
            ...
            if (MoveParams.IsWallJumping)
            {
                if (IsClimbJumping)
                {
                    ...
                    moveValue = HorizontalDirection3 *
                                (inputMagnitudeAmplified * MaxLength / (MaxHeightTime + FallingTime));
            }
            else
                moveValue = HorizontalDirection3 *
                            (inputMagnitudeAmplified * MaxLength / (MaxHeightTime + FallingTime));
            ...
            if (!IsLeapEnd)
            {
                var rayHead = new Ray(transform.position + Vector3.up * characterControllerEnveloper.Height / 2,
                    Vector3.up);
                var isHeadHit = Physics.Raycast(rayHead.origin, rayHead.direction,
        ...
        public override Vector3 CameraTargetUpdate()
        {
            ...
            if (!IsBlocked)
            {
                ...
                if (moveDirection != Vector3.zero)
                {
                    var verticalRay =
                        new Ray(
                            InitialPosition + Vector3.up * MaxJumpHeight +
                            moveDirection * MaxLength / (MaxHeightTime + FallingTime), Vector3.down * MaxJumpHeight);
                ...
                var dir = (transform.position + moveDirection * (camTargetMoveAmount * inputDirSpeed * dotMag) -
                           moveCameraTarget.transform.position).XYZ3toX0Z3() + Vector3.up * yRevision;
