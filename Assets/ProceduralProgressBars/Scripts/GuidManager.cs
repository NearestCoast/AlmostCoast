...
namespace Renge.PPB {
    ...
    public class GuidManager {
        ...
        private struct GuidInfo {
            ...
            public GuidInfo(UniqueMonoBehaviour comp) {
                go = comp.gameObject;
                OnRemove = null;
                OnAdd = null;
            }
        ...
        public static bool Add(UniqueMonoBehaviour uniqueMonoBehaviour) {
            if (Instance == null) {
                Instance = new GuidManager();
            }

            return Instance.InternalAdd(uniqueMonoBehaviour);
        }
        ...
        private bool InternalAdd(UniqueMonoBehaviour uniqueMonoBehaviour) {
            Guid guid = uniqueMonoBehaviour.GetGuid();

            GuidInfo info = new GuidInfo(uniqueMonoBehaviour);
            ...
            if (existingInfo.go != null && existingInfo.go != uniqueMonoBehaviour.gameObject) {
                // normally, a duplicate GUID is a big problem, means you won't necessarily be referencing what you expect
                //if (Application.isPlaying) {
                //    Debug.AssertFormat(false, uniqueMonoBehaviour, "Guid Collision Detected between {0} and {1}.\nAssigning new Guid. Consider tracking runtime instances using a direct reference or other method.", (guidToObjectMap[guid].go != null ? guidToObjectMap[guid].go.name : "NULL"), (uniqueMonoBehaviour != null ? uniqueMonoBehaviour.name : "NULL"));
                //} else {
                ...
                //    Debug.LogWarningFormat(uniqueMonoBehaviour, "Guid Collision Detected while creating {0}.\nAssigning new Guid.", (uniqueMonoBehaviour != null ? uniqueMonoBehaviour.name : "NULL"));
                //}
                return false;
            }
