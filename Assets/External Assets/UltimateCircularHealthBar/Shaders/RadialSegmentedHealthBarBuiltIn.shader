Shader "Renge/RadialSegmentedHealthBarBuiltIn" {
	Properties{
		...
		[HDR] _PulseColor("PulseColor", Color) = (1,1,1,1)
	...
	SubShader{
		...
		Pass {
			...
			float4 gradient(float4 colors[8], float colorCount, float2 alphas[8], float alphaCount, float t, bool isFixed)
			{
				if (isFixed)
				{
					int index = 0;
			...
			fixed4 frag(VertexOut i) : SV_Target
			{
				...
				float fragmentLineWidth = _LineWidth * tex2D(_VariableWidthCurve, float2(remap(calc000, float2(0, pi2 - arcpi2), float2(0, 1)), 0)).r;
				...
				round(clamp(remap(_SegmentCount, float2(0, 1), float2(0, 0.5)), 0, 1)))), 0, 1);
				...
				//				)
				//		));
				float pulsating = _PulsateWhenLow * remap(sin(_Time * _PulseSpeed * 20), float2(-1, 1), float2(0, 1 - clamp(remap(_SegmentCount - _RemoveSegments, float2(0, _SegmentCount * _PulseActivationThreshold), float2(0, 1)), 0, 1)));
				//}
				...
				//lerp based on pulse
				float4 innerColor = lerp(_InnerColor, _PulseColor, pulsating);
				...
				//lerp based on pulse
				gradientModifiedInnerColor = lerp(gradientModifiedInnerColor, _PulseColor, pulsating);
				...
				//NOISE
				float innerNoise = 
					_ContentNoiseEnabled * myNoise(i.uv, _ContentNoiseOffset, _ContentNoiseScale, _ContentNoiseStrength, _ContentNoiseEnabled) +
				...
				float emptyNoise = _EmptyNoiseEnabled * myNoise(i.uv, _EmptyNoiseOffset, _EmptyNoiseScale, _EmptyNoiseStrength, _EmptyNoiseEnabled) +
				...
				float overlayNoise = _OverlayNoiseEnabled * myNoise(i.uv, _OverlayNoiseOffset, _OverlayNoiseScale, _OverlayNoiseStrength, _OverlayNoiseEnabled) +
				...
				round(clamp(remap(_SegmentCount, float2(0, 1), float2(0, 0.5)), 0, 1))));
